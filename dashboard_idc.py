...
# Cache the addresses list
@st.cache_data
def get_all_addresses(db_path: str = "addresses.db") -> list:
    """
...
if search_term:
    filtered_df = df[df["address"].str.contains(search_term, case=False, na=False)]
else:
    filtered_df = df

# Create the dropdown with filtered addresses
if not filtered_df.empty:
    selected_address = st.selectbox(
        "Select an address", options=filtered_df["address"].tolist()
    )

    # Show EGID when an address is selected
    if selected_address:
        egid = filtered_df[filtered_df["address"] == selected_address]["egid"].iloc[
            0
        ]
        st.success(f"EGID for {selected_address}: {egid}")
else:
    st.warning("No addresses found matching your search.")
