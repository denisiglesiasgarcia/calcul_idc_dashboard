...
def _init_all_tables():
    ...
    db.init_reseau_thermique_table()
    db.init_idc_table()
...
def _patch_refreshes(monkeypatch, calls):
    ...
    monkeypatch.setattr(
        ...
        lambda **kw: (
            calls.__setitem__("reseau_thermique", calls["reseau_thermique"] + 1) or 1
        ),
    )
    monkeypatch.setattr(
        db,
...
class TestRefreshDbAtStartupIfNeeded:
    def test_refreshes_when_no_previous_timestamp(self, tmp_path, monkeypatch):
        ...
        calls = {"addr": 0, "autor": 0, "batiments": 0, "idc": 0, "reseau_thermique": 0}
